...
void main()
{
  ...
  // float spec = pow(max(dot(viewDir, reflectDir), 0.0), u_material.shininess); // Phong
  vec3 halfwayDir = normalize(lightDir + viewDir); // Blinn-Phong
  float spec = pow(max(dot(v_normal, halfwayDir), 0.0), u_material.shininess); // Blinn-Phong
  vec3 specularColor = u_pointLight.specular * spec * u_color;
