...
void Model::draw(const glm::mat4 &projectionMatrix, const glm::mat4 &viewMatrix, const std::shared_ptr<Shader>& shader,
        ...
        if (materialId != -1) {
            ...
            if (it != m_materialsMap.end()) {
                ...
                if (hasDiffuseTexture) {
                    material->diffuseTexture = std::make_shared<Texture>(
                            modelRootPath + materials[materialId].diffuse_texname);
                }

                if (hasSpecularTexture)
                    material->specularTexture = std::make_shared<Texture>(
                            modelRootPath + materials[materialId].specular_texname);
                if (hasNormalTexture)
                    material->normalTexture = std::make_shared<Texture>(
                            modelRootPath + materials[materialId].normal_texname);
